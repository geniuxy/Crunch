...
void UPlayerTeamLayoutWidget::NativeConstruct()
{
	...
	{
		UPlayerTeamSlotWidget* NewSlotWidget =
			CreateWidget<UPlayerTeamSlotWidget>(GetOwningPlayer(), PlayerTeamSlotWidgetClass);

		TeamSlotWidgets.Add(NewSlotWidget);

		UHorizontalBoxSlot* NewSlot;
		if (i < UNetFunctionLibrary::GetPlayerCountPerTeam())
		{
...
void UPlayerTeamLayoutWidget::UpdatePlayerSelection(const TArray<FPlayerSelection>& PlayerSelections)
{
	...
	for (const FPlayerSelection& PlayerSelection : PlayerSelections)
	{
		...
		TeamSlotWidgets[PlayerSelection.GetPlayerSlot()]->UpdateSlot(
			PlayerSelection.GetPlayerNickName(), PlayerSelection.GetCharacterDefinition()
		);
	}
}
